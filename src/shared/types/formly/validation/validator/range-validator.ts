 ...
 *
 */
export function rangeValidator(control: AbstractControl, config: FormlyFieldConfig, options = {},) {

  const start = control.get(options['start'])?.value;
  const end = control.get(options['end'])?.value;

  if (start === undefined || start === '' || end === undefined || end === '') {
    return null;
  }

  const type = !options['type'] && config.get !== undefined ? config.get(options['start']).type : options['type'];

  let normalizer = getNormalizer(type);

  if (normalizer(start) < normalizer(end)) {
    return null;
  }

  return {range: {message: options['message'] || 'Invalid range!', errorPath: options['errorPath'] || options['end']}};
}
